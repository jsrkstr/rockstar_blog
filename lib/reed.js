...
function openRedis(callback) {
	...
	client = require("redis-node").createClient(redisConf.port, redisConf.host);
	...
	if (redisConf.password) {
		...
		// client2.auth(redisConf.password, function (err) {
		// 	if (err) return reed.emit('error', err);
		// 	console.log("client2 authed");
		// 	callback(true);
		// });
